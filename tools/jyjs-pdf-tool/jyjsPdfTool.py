...
#  py ./index.py  -c "#FF0000" -o 0.3 -s 20 -x 180 -y 0 -w "test-watermark" -d output-mark.pdf input.pdf
def mark(filename, watermark, font_name, font_size, color, opacity,x, y, destination_file_name, water_page_number, mark_count, columns):
...
@click.option('-ty', '--type', default='mark', help='multi or single')  # mark topdf
@click.option('-df', '--docx-file')  # mark topdf
def annotate(filename, watermark, font_name, font_size, color, opacity,x, y, destination_file_name, water_page_number, mark_count, columns, type, docx_file):
    # py .\index.py -ty topdf -df input.docx output.pdf
    if type == 'topdf':
        convert(docx_file, filename)
    else:
        mark(filename, watermark, font_name, font_size, color, opacity,x, y, destination_file_name, water_page_number, mark_count, columns)
